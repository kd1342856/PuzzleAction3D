...
{
	if (!m_active) return;
	SyncMouseAnchorOnGrabBegin();
	if (EngineCore::Engine::Instance().IsMouseGrabbed()) 
	{
		UpdateRotateByMouse();
	}
	// ターゲットの行列(有効な場合利用する)
	...
	if (auto ent = m_wpTargetEnt.lock())
	{
		if (ent->HasComponent<TransformComponent>()) 
		{
			_targetMat = Math::Matrix::CreateTranslation(ent->GetComponent<TransformComponent>().GetPos());
			hasTarget = true;
		}
	}
	else if (_spTarget)
